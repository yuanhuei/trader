...
void Gatewaymanager::Init()
{
	...
	m_gatewaymap.insert(std::pair<std::string, std::shared_ptr<JSGateway>>("CTP", ctpgateway));
	//IB
...
void Gatewaymanager::exit()
{
	...
	for (std::map<std::string, std::shared_ptr<JSGateway>>::iterator it = m_gatewaymap.begin(); it != m_gatewaymap.end(); it++)
	{
		it->second->close();
	}
	m_gatewaymap.clear();
